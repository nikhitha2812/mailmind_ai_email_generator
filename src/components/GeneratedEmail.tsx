...
import { Check, Copy, RefreshCw, Sparkles, Wand2, X } from 'lucide-react';
  ...
  if (isLoading) {
    return (
      <div className="flex h-full min-h-[400px] flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <p className="text-lg font-semibold text-slate-700">Generating your email...</p>
        <p className="mt-1 text-sm text-slate-400">The AI is crafting the perfect message</p>
      </div>
    );
  }
  ...
  if (!email) {
    return (
      <div className="flex h-full min-h-[400px] flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50/50 p-8 text-center">
        ...
        <p className="text-lg font-semibold text-slate-600">Your email will appear here</p>
        <p className="mt-1 max-w-sm text-sm text-slate-400">
          Fill in the details on the left and click Generate to create a professional email with AI.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-in-up rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-xl font-bold text-slate-900">Generated Email</h2>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button
          onClick={onImprove}
          className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-blue-500/20 transition hover:shadow-lg hover:shadow-blue-500/30"
        >
